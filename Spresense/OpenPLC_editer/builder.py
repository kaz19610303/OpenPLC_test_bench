...
def build(st_file, platform, source_file, port, txtCtrl, update_subsystem):
    ...
    if (update_subsystem):
        ...
        # Add Spresense Board
        compiler_logs += runCommand(
            cli_command + ' config add board_manager.additional_urls https://github.com/sonydevworld/spresense-arduino-compatible/releases/download/generic/package_spresense_index.json')
        wx.CallAfter(txtCtrl.SetValue, compiler_logs)
        wx.CallAfter(scrollToEnd, txtCtrl)
